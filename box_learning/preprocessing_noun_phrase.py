...
def find_question_strings(file_index):
	question_st = []
	input_file = open("data/spilit/" + str(file_index) + '_lemma.txt','r')
	for line in input_file:
		if '?' in line:
...
def added_np_with_starting_num(question_strings_np):
	for np in question_strings_np:
		np_parts = np.split(' ')
		if is_word_number(np_parts[0]) == True and len(np_parts) >1:
			rest_np = np[np.index(' ') + 1:]
			if rest_np not in question_strings_np:
				question_strings_np.append(rest_np)
		elif '-' in np:
			rest_np = np[np.index('-') + 1:]
			if rest_np not in question_strings_np:
				question_strings_np.append(rest_np)
	return question_strings_np

def omit_long_np(question_strings_np):
	res = []
	for np in question_strings_np:
		if len(np.split(' ')) <= 5:
			if not np in res:
...
word_list_not_in_end = ['combined', 'combine', 'altogether', 'cost', 'total', 'selling', 'of', 'some', 'apart', 'more', 'and']
...
if __name__ =="__main__":
	...
	for i in range(0, 514):
		...
		input_whole_question = open('data/problems/' + str(i) + '_lemma.txt', 'r')
		whole_question = input_whole_question.readline().lower()
		output_file = open("data/np_preprocessed_no_article_entity/" + str(i) + '.txt','w')
		res_no_long_np = omit_long_np(question_strings_nps)
		res_unit_and = check_for_unit_and_rule(res_no_long_np)
		...
		res_added_np_with_starting_num = added_np_with_starting_num(res_no_ban_word)
		print res_added_np_with_starting_num
		res_no_names = omit_names(res_added_np_with_starting_num, names_list)
