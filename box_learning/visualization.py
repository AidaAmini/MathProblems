...
def read_samples(num_of_iterations, tp_samples, fp_samples, tn_samples, fn_samples, th_list):
	for i in range(0, num_of_iterations):
		...
		line = input_file.readline()
		while line != '':
			if 'test_procedure for problems is ended' in line:
				break
			if 'sampling' in line:
				line = input_file.readline()
				parts = line[:-1].split('	')
				...
				chain.append(parts[5])
				if parts[3] == '[\'child ticket\']' and parts[4] == '[\'ticket\']':
					print 'sampleeeeeeeeeeeee'
					print y_test
					print y_predicted
				if y_test == 1:
					if y_predicted == 1:
...
# print tp_samples
# print h
plt.plot(recal[0], precision[0], '.')
plt.show()
# plt.plot(recal[1], precision[1], '.')
# plt.show()
# plt.plot(recal[2], precision[2], '.')
# plt.show()
