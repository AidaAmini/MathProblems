...
def refine_articles(question_strings_np):
	...
	for noun_phrase in question_strings_np:
		if ' cost ' in noun_phrase:
			continue
		parts = noun_phrase.split(' ')
		if parts[0] == 'a' or parts[0] == 'an' or parts[0] == 'one' or parts[0] == 'the' :
			rest_noun = ''
		...
		elif len(parts) >1 and parts[0] == 'and' and (parts[1] == 'a' or parts[1] == 'an' or parts[1] == 'one' or parts[1] == 'the'):
			rest_noun = ''
...
def omit_math_modifiers(question_strings_np):
	modifier_element_list = [ 'triple', 'more', 'less', 'twice', 'some', 'only','any', 'many', 'number', 'total', 'time', 'much', 'amount', 'how', 'where', 'one', 'two', 'three', 'four', 'five','six','seven','eight', 'nine', 'ten','hundred']
...
def find_question_strings(file_index):
	question_st = []
	input_file = open("data/spilit_preprocessed2/" + str(file_index) + '_lemma.txt','r')
	for line in input_file:
		if '?' in line:
...
if __name__ =="__main__":
	...
	for i in range(0, 400):
		...
		if i == 17 or i == 30 or i == 33 or i == 70 or i == 104 or i == 106 or i ==132 or i == 151 or i == 215 or i == 246 or i == 200 or i == 281 or i == 283:
			continue
		input_file = open("data/np_pos2/" + str(i) + '_lemma.txt','r')
		...
		input_whole_question = open('data/problem_preprocessed2/' + str(i) + '_lemma.txt', 'r')
		whole_question = input_whole_question.readline().lower()
		output_file = open("data/np_preprocessed_no_article_entity2/" + str(i) + '_lemma.txt','w')
		res_article = refine_articles(question_strings_nps)
		res_q_string = omit_question_strings(res_article)
		res_cost_omitted = omit_cost(res_q_string)
		res_no_math_modifier = omit_math_modifiers(res_cost_omitted)
		...
		res_no_place = omit_places(res_no_modifier_in_end, "data/pos2/" + str(i) + "_lemma.txt", i)
